  ...
  this.docModel.findOne(
    ...
    function(err, doc) {
      ...
      for (var i = 0; i < keysLength; i++) {
        if (self.request.body[keys[i]] !== undefined && keys[i] !== campaigns) {
          // Convert timestamps to Date object
          // @todo: support date as string that includes timezone details
          if (keys[i] == drupal_register_timestamp || keys[i] == birthdate_timestamp) {
            var timestamp = parseInt(self.request.body[keys[i]]);
            updateArgs['$set'][keys[i]] = convertToDate(timestamp);
          }
