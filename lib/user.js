  ...
  this.docModel.findOne(
    ...
    function(err, doc) {
      ...
      if (self.request.body.first_name !== undefined) {
        updateArgs.first_name = self.request.body.first_name;
      }
      else if (doc && doc.first_name !== undefined) {
        updateArgs.first_name = doc.first_name;
      }

      if (self.request.body.last_name !== undefined) {
        updateArgs.last_name = self.request.body.last_name;
      }
      else if (doc && doc.last_name !== undefined) {
        updateArgs.last_name = doc.last_name;
      }
      ...
      if (self.request.body.drupal_register_timestamp !== undefined) {
        ...
        // Store month and year as human readable instead of 0-indexed or starting at 1900.
        var regMonth = (drupalRegisterDate.getMonth()) + 1;
        var regYear = (drupalRegisterDate.getYear()) + 1900;
        ...
        updateArgs.drupal_register_year = regYear;
      }
      ...
      if (self.request.body.birthdate_timestamp !== undefined) {
        ...
        // Store month and year as human readable instead of 0-indexed or starting at 1900.
        var bdayMonth = parseInt(birthdate.getMonth()) + 1;
        var bdayYear = parseInt(birthdate.getYear()) + 1900;
        ...
        updateArgs.birthdate_year = bdayYear;
      }
  ...
  this.docModel.remove(
    ...
    function (err, num) {
      ...
      if (num == 0) {
        ...
        console.log('Found no document to delete for email: %s', self.email);
      }
      else {
        self.response.send(200, 'Delete ' + num + ' document(s) for email: ' + self.email);
        console.log('Delete %d document(s) for email: %s', num, self.email);
      }
    }
  );
};
