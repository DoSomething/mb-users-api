  ...
  this.docModel.findOne(
    ...
    function(err, doc) {
      ...
      for (var i = 0; i < keysLength; i++) {
        ...
        if (self.request.body[keys[i]] !== undefined && keys[i] !== "campaigns" && keys[i] !== "subscriptions") {
      ...
      if (self.request.body.subscriptions !== undefined) {
        var reqSubscriptions = self.request.body.subscriptions;
        var reqSubscriptionsLength = reqSubscriptions.length;

        for (var i = 0; i < reqSubscriptionsLength; i++) {

          if (reqSubscriptions.mailchimp !== undefined) {
            updateArgs.reqSubscriptions.mailchimp = reqSubscriptions.mailchimp;
          }
          if (reqSubscriptions.digest !== undefined) {
            updateArgs.reqSubscriptions.digest = reqSubscriptions.digest;
          }
          if (reqSubscriptions.user_events !== undefined) {
            updateArgs.reqSubscriptions.user_events = reqSubscriptions.user_events;
          }
