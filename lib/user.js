...
User.prototype.post = function(req, res) {
  ...
  this.docModel.findOne(
    ...
    function(err, doc) {
      ...
      if (self.request.body.anniversary !== undefined) {
        // Convert timestamp string to Date object
        var timestamp = parseInt(self.request.body.anniversary);
        var anniversary = new Date(timestamp);

        // Structure for anniversary in user model
        updateArgs.anniversary = {
          day: anniversary.getDate(),
          month: anniversary.getMonth()
        };
        updateArgs.anniversary_year = anniversary.getYear();
      }
      else if (doc && typeof doc.anniversary == 'object' && !isObjectEmpty(doc.anniversary)) {
        updateArgs.anniversary = doc.anniversary;
        updateArgs.anniversary_year = doc.anniversary_year;
      }

      if (self.request.body.birthdate !== undefined) {
        // Convert timestamp string to Date object
        var timestamp = parseInt(self.request.body.birthdate);
