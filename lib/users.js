 ...
 */
var getAllUsers = function(request, response, objUsers) {
  var self = objUsers;
  self.request = request;
  self.response = response;

  var conditions = {};
  // Only return subscribed users unless told otherwise.
  ...
  // Page size.
  var pageSize = objUsers.defaults.pageSize;
  ...
  var limit = objUsers.defaults.limit;
  ...
  // Execute the query.
  var query = objUsers.docModel.find(conditions).skip(skip).limit(limit);
 ...
 *   Reference to the Users object that'll execute the query.
 */
var getByBirthdate = function(request, response, objUsers) {
  var query = {};
  ...
  objUsers.docModel.aggregate(query).exec(
    function(err, docs) {
      if (err) {
        self.response.send(500, err);
        dslogger.error(err);
 ...
 *   Reference to the Users object that'll execute the query.
 */
var getByDrupalRegisterDate = function(request, response, objUsers) {
  var query = {};
...
Users.prototype.get = function(request, response) {
  if (typeof(request.query.birthdate) !== 'undefined') {
    getByBirthdate(request, response, this);
  }
  else if (typeof(request.query.drupal_register_date) !== 'undefined') {
